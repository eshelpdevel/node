...
############################################################################################################### 
	...
	function blast_sync2($conn,$param){
		...
		$files = array_slice($files, 0, $param);
...
$param=0;
$myfile = fopen("param_txt.txt", "r") or die("Unable to open file!");
$param  = (int)fread($myfile,filesize("param_txt.txt"));
fclose($myfile);
if ($param==0||$param=='') {
	$param=20;
}	   
blast_sync2($conn,$param);		
